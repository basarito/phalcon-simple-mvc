...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        <form method="post" action="<?= $this->url->get('member/update/' . $member->member_id) ?>" id="saveMemberForm">
                                ...
                                <div class="input-group">
                                    <span class="input-group-addon info">Datum rodjenja</span>
                                        <input type="text" class="form-control" name="date_of_birth"
                                               aria-describedby="basic-addon3" placeholder="npr. 1989-05-03"
                                ...
                                <div class="input-group">
                                    <span class="input-group-addon info">Generacija</span>
                                        <input type="text" class="form-control" name="class"
                                               aria-describedby="basic-addon3" placeholder="npr. 2013"
                                               value="<?= $member->class ?>" required>
                                </div>
                            </span>
                        </div>
                    </div>
                        ...
                        <select class="form-control" id="selectCompany" name="company_id">
                            ...
                            <?php if ($this->length($companies) > 0) { ?>
                                <?php foreach ($companies as $company) { ?>
                                    <?php if ($company->company_id == $member->company) { ?>
                                        <option value="<?= $company->company_id ?>" selected>
                                            <?= $company->name ?> (<?= $company->City->name ?>, <?= $company->City->country ?>)
                                        </option>
                                    <?php } else { ?>
